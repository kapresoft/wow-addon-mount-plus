...
local function __categories(key, val)
    if key then MOUNTP_DEBUG_ENABLED_CATEGORIES[key] = val end
    return MOUNTP_DEBUG_ENABLED_CATEGORIES or {}
end
local function __category(key)
    MOUNTP_DEBUG_ENABLED_CATEGORIES = MOUNTP_DEBUG_ENABLED_CATEGORIES or {}
    return MOUNTP_DEBUG_ENABLED_CATEGORIES[key]
end
--- @param val number|nil Optional log level to set
--- @return number The new log level passed back
local function __logLevel(val)
    if val then MOUNTP_LOG_LEVEL = val end
    return MOUNTP_LOG_LEVEL or 0
end
